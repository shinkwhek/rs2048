...
impl Board {
    ...
    fn format_line(&self, n: usize, t: Option<usize>) -> String {
        ...
        if self.tiles[n].is_exist() {
            if t.is_none() {
                ...
                return Colour::White
                    .on(color)
                    .fg(if color == Colour::RGB(234, 222, 207)
                        || color == Colour::RGB(231, 218, 187)
                    {
    ...
    pub fn fin(&mut self) -> bool {
        ...
        println!(
            "{} {}",
            Colour::White.bold().underline().paint("Total score:"),
            self.get_score()
        );
